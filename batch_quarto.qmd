...
batch_data <- read_excel(batch_input_file)
...
batch_data$sample_name_combined <- paste0(batch_data$number, "_", batch_data$`Label`)
...
input_ab1 <- file.path(input_ab1, experiment_file)

data <- read.abif(input_ab1)
#str(data)
#class(int)
...
```{=html}
<div style="overflow-x: auto; white-space: nowrap;">
  <img src="alignment.png" width="{{ wdt }}">
</div>
```
...
names(dna_set) <- alignment_data$`Indel Label`  # Name the sequences
