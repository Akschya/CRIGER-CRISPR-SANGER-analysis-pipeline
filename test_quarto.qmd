...
options(warn = 1)  # Show warnings temporarily for debugging
options(Ncpus = 1)  # Use a single CPU to avoid multithreading messages
...
# Check if the 'packages.yml' file exists
if (file.exists("packages.yml")) {
  packages_info <- yaml::read_yaml("packages.yml")
  
  # Install CRAN packages (base packages)
  if (!is.null(packages_info$base)) {
    required_base_packages <- packages_info$base
  ...
  if (!is.null(packages_info$bioconductor)) {
    required_bioconductor_packages <- packages_info$bioconductor
    # Ensure BiocManager is available
    if (!requireNamespace("BiocManager", quietly = TRUE)) {
      install.packages("BiocManager")
    }
    for (pkg in required_bioconductor_packages) {
      if (!require(pkg, character.only = TRUE)) {
